using Microsoft.AspNetCore.Components;
using WebApp.Models;
using WebApp.Services;
using WebApp.ViewModels;

namespace WebApp.Components.Pages;

public partial class Materials : ComponentBase
{
    [Inject] private MaterialsViewModel VM { get; set; } = default!;
    [Inject] private CustomAuthStateProvider AuthStateProvider { get; set; } = default!;

    private List<RawMaterialDto> _materials => VM.Materials;
    private bool _isLoading => VM.IsLoading;
    private bool _isSaving => VM.IsSaving;
    private string? _errorMessage => VM.ErrorMessage;
    private RawMaterialCreateRequest _newMaterial => VM.NewMaterial;
    private RawMaterialUpdateRequest _editRequest => VM.EditRequest;
    private RawMaterialDto? _selectedMaterial => VM.SelectedMaterial;
    private RawMaterialDto? _editMaterial => VM.EditMaterial;
    private RawMaterialDto? _materialToDelete => VM.MaterialToDelete;
    private decimal _newStockQuantity
    {
        get => VM.NewStockQuantity;
        set => VM.NewStockQuantity = value;
    }
    private bool _showCreateModal => VM.ShowCreateModal;
    private bool _showStockModal => VM.ShowStockModal;
    private bool _showEditModal => VM.ShowEditModal;
    private bool _showDeleteModal => VM.ShowDeleteModal;

    // Pagination
    private int _currentPage => VM.CurrentPage;
    private int _totalPages => VM.TotalPages;
    private int _totalCount => VM.TotalCount;
    private bool _hasPreviousPage => VM.HasPreviousPage;
    private bool _hasNextPage => VM.HasNextPage;

    private bool _dataLoaded;

    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender && !_dataLoaded)
        {
            await AuthStateProvider.GetAuthenticationStateAsync();
            await VM.LoadAsync();
            _dataLoaded = true;
            StateHasChanged();
        }
    }

    private void OpenCreateModal() => VM.OpenCreate();
    private void CloseCreateModal() => VM.CloseCreate();
    private async Task CreateMaterial() => await VM.CreateAsync();
    private void OpenStockModal(RawMaterialDto m) => VM.OpenStock(m);
    private void CloseStockModal() => VM.CloseStock();
    private async Task UpdateStock() => await VM.UpdateStockAsync();
    private void OpenEditModal(RawMaterialDto m) => VM.OpenEdit(m);
    private void CloseEditModal() => VM.CloseEdit();
    private async Task UpdateMaterial() => await VM.UpdateAsync();
    private void OpenDeleteModal(RawMaterialDto m) => VM.OpenDelete(m);
    private void CloseDeleteModal() => VM.CloseDelete();
    private async Task DeleteMaterial() => await VM.DeleteAsync();
    private async Task GoToPage(int page) { await VM.GoToPageAsync(page); StateHasChanged(); }
    private async Task NextPage() { await VM.NextPageAsync(); StateHasChanged(); }
    private async Task PreviousPage() { await VM.PreviousPageAsync(); StateHasChanged(); }
}
